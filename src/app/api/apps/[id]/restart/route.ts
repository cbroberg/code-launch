...
export async function POST(
  ...
  if (agent) {
    ...
    const event = await sendCommand({ type: "restart", requestId: crypto.randomUUID(), app: toAppConfig(app) });
  ...
  try {
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Failed to restart" }, { status: 500 });
  }
}
