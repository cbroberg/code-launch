...
export async function POST(
  ...
  // Mark as manually stopped so auto-boot never restarts it without user action
  await db.update(apps).set({ manualStop: true, updatedAt: new Date().toISOString() }).where(eq(apps.id, appId));
